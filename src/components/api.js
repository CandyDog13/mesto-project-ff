...
// Для эксперимента заменил данную функцию на реализацию с помощтю getData
...
function getData(endOfUrl, method = "GET") {
  console.log(`${config.baseUrl}${endOfUrl}`);
  return fetch(`${config.baseUrl}${endOfUrl}`, {
    headers: config.headers,
  }).then(checkRes);
}
